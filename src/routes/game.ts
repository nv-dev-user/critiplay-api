...
import {
  ...
  getGameBySlug,
  getGamesByFilters,
} from "@/db/repositories/games.repository";
...
export const gameListHandler = async (c: Context<Env>) => {
  ...
  const games: Game[] = await getGamesByFilters(
    { partialTitle, categoryId, ownerIds, tagIds },
    limit,
    offset,
  );
...
export const gameDeletionHandler = async (c: Context<Env>) => {
  ...
  if (game.createdByProfileId) {
    ...
    if (!user) {
      return c.json(
        { message: "The user who created this game could not be found." },
        404,
      );
    }
    ...
    if (!organization) {
      return c.json(
        {
          message:
            "The organization that created this game could not be found.",
        },
        404,
      );
    }

    const role = await getUserRoleInOrganization(
  ...
  return c.json(
    { message: `Game with slug ${gameSlug} deleted successfully!` },
    200,
  );
};
...
export const gamePatchHandler = async (c: Context<Env>) => {
  ...
  return c.json(
    {
      message: `Game with slug ${gameSlug} updated successfully!`,
      data: requestBody,
    },
    200,
  );
